...
class MapScene(Scene):
    ...
    def _draw_hover_popup(self, screen):
        ...
        if popup_x + popup_w > settings.WINDOW_WIDTH - 10:
            popup_x = mx - popup_w - 20
        if popup_y + popup_h > settings.WINDOW_HEIGHT - 10:
        ...
        type_label = type_labels.get(room.objective if room.objective == "capture_flag" else room.type, "ROOM")
        ...
        title = pygame.font.Font(None, 22).render(t(room.name), True, settings.WHITE)
        screen.blit(title, (popup_x + 12, popup_y + 10))

        badge_txt = pygame.font.Font(None, 15).render(type_label, True, type_color)
        screen.blit(badge_txt, (popup_x + 12, popup_y + 34))

        pygame.draw.line(screen, (40, 40, 60),
                         (popup_x + 10, popup_y + 52),
                         (popup_x + popup_w - 10, popup_y + 52), 1)
        ...
        diff_txt = pygame.font.Font(None, 16).render(f"{diff_name}", True, diff_color)
        screen.blit(diff_txt, (popup_x + 12, popup_y + 58))

        star_y = popup_y + 58
        star_x = popup_x + popup_w - 20 - room.difficulty * 15
        for i in range(room.difficulty):
            pts = []
            for ai in range(10):
                ang = math.radians(ai * 36 - 90)
                r = 5 if ai % 2 == 0 else 2
                pts.append((star_x + i * 15 + r * math.cos(ang),
                             star_y + r * math.sin(ang)))
            pygame.draw.polygon(screen, diff_color, pts)

        gold_txt = pygame.font.Font(None, 16).render(f"+{room.gold_reward} gold", True, settings.GOLD)
        screen.blit(gold_txt, (popup_x + 12, popup_y + 80))
        ...
        state_txt = pygame.font.Font(None, 16).render(state_lbl, True, state_col)
        screen.blit(state_txt, (popup_x + popup_w - state_txt.get_width() - 12, popup_y + 80))

        narr_font = pygame.font.Font(None, 13)
        narr_full = t(room.narrative)
        narr_text = narr_full[:55] + "…" if len(narr_full) > 55 else narr_full
        narr_surf = narr_font.render(narr_text, True, (140, 140, 160))
        screen.blit(narr_surf, (popup_x + 12, popup_y + 102))

        if room. enemies:
            enemy_strs = []
            for etype, count in room.enemies:
                enemy_strs.append(f"{count}x {etype}")
            enemy_text = "  ".join(enemy_strs)[:50]
            enemy_surf = pygame.font.Font(None, 12).render(enemy_text, True, (180, 120, 60))
            screen.blit(enemy_surf, (popup_x + 12, popup_y + 120))
