...
import settings
from utils import draw_text, clamp, distance, angle_between
...
class Game:
    ...
    def _try_rewind(self):
        ...
        if result:
            ...
            healed = max_hp - hp
            if hp > old_hp:
                self.floating_text.add_heal(self.player.x, self.player.y, int(hp - old_hp))
            ...
            self.particles.emit_burst(
                self.player.x, self.player.y, settings.CYAN, 30, 100, 0.6
            )
    ...
    def _update_playing(self, dt):
        ...
        if len(alive) == 0 and self.wave_countdown <= 0:
            ...
            if self.current_wave == len(settings.WAVES) - 1:
                self.state = "VICTORY"
                self._on_victory()
            else:
