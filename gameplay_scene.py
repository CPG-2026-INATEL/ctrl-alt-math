...
class GameplayScene(Scene):
    def __init__(self, game):
        super().__init__(game)
        self.state = "WAVE_INTRO"  # WAVE_INTRO, PLAYING, WAVE_COMPLETE
    ...
    def _update_playing(self, dt):
        ...
        if len(alive) == 0 and self.game.wave_countdown <= 0:
            ...
            if self.game.current_wave == len(settings.WAVES) - 1:
                self.game.scene_manager.switch("victory")
            else:
    ...
    def _try_rewind(self):
        ...
        if result:
            pos, hp, max_hp = result
            old_hp = self.game.player.hp
            self.game.player.set_position(pos)
            self.game.player.set_hp(hp, max_hp)
            if hp > old_hp:
                self.game.floating_text.add_heal(
                    self.game.player.x, self.game.player.y, int(hp - old_hp)
                )
    ...
    def _draw_game(self, screen):
        ...
        if self.state == "WAVE_INTRO":
            self.game.ui.draw_wave_intro(
                screen, settings.WAVES[self.game.current_wave],
                self.game.current_wave + 1
            )
        elif self.state == "WAVE_COMPLETE":
